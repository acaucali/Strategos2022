...
export class ProyectosComponent implements OnInit {
  ...
  constructor(
    ...
    private organizacionesService: OrganizacionStrategosService,
    private tiposService: TipoProyectoStrategosService,
  ...
  crearProyecto() {
    ...
    this.proyectoSeleccionado.estatusId = 6;
  ...
  getProyectos() {
    ...
    if (this.isAdmin == true) {
      this.proyectoService
        .getProyectosListTipoId(false)
        .subscribe((response) => {
          this.proyectos = response;
        });
    } else {
      this.proyectoService
        .getProyectosListOrgTipoId(this.organizacionId, false)
        .subscribe((response) => {
          this.proyectos = response;
        });
    }
  ...
  abrirModal(proyecto: Proyectos) {
    ...
    if (
      ...
      this.proyectoSeleccionado.proyectoId != null
    ) {
      this.proyectoService
        .getPoblacionesListId(this.proyectoSeleccionado.proyectoId)
        .subscribe((response) => {
          this.proyectoSeleccionado.poblaciones = response;
        });
      console.log(this.proyectoSeleccionado);
    }
    this.modalService.abrirModal();
  ...
  descargarPDF(preproyecto: Proyectos) {
    window.open(
      URL_BACKEND +
        '/api/strategos/bancoproyectos/preproyecto/pdf/' +
        preproyecto.proyectoId
    );
  }
  ...
  descargarPDFResumido() {
    if (this.isAdmin == true) {
      window.open(
        URL_BACKEND +
          '/api/strategos/bancoproyectos/preproyecto/pdf/resumido/' +
          0
      );
    } else {
      window.open(
  ...
  descargarXls(preproyecto: Proyectos) {
    window.open(
      URL_BACKEND +
        '/api/strategos/bancoproyectos/idea/excel/' +
        preproyecto.proyectoId
    );
  }
  ...
  descargarXlsResumido() {
    if (this.isAdmin == true) {
      window.open(
        URL_BACKEND + '/api/strategos/bancoproyectos/preproyecto/excel/' + 0
      );
    } else {
      window.open(
        URL_BACKEND +
          '/api/strategos/bancoproyectos/preproyecto/excel/' +
          this.organizacionId
      );
    }
  }
