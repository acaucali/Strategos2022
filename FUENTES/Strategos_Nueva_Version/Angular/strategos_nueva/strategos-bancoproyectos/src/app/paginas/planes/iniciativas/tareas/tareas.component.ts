...
export class TareasComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(localStorage.getItem('objetivoId') != null){
      ...
      this.actividadService.getActividadesProyecto(Number(localStorage.getItem('actividad'))).subscribe(response => {this.tareas = response});
  ...
  eliminarTarea(tar: Actividad){
    swal.fire({
      ...
      if (result.isConfirmed) {
        this.actividadService.delete(tar.actividadId).subscribe(
          response =>{
            this.getTareas();
            swal.fire(
              'Actividad eliminada!',
