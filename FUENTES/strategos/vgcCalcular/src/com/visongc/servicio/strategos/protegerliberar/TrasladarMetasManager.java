...
public class TrasladarMetasManager {
	  ...
	  //
	  public TrasladarMetasManager(String[][] configuracion, StringBuffer log, VgcMessageResources messageResources)
	  {
	  ...
	  public TrasladarMetasManager(PropertyCalcularManager pm, StringBuffer log, VgcMessageResources messageResources, Servicio servicio)
	  {
	    this.pm = pm;
	    this.log = log;
	    this.messageResources = messageResources;
	  ...
	  //  revisar
	  public boolean Ejecutar(List<Indicador> indicadores, Calendar fecha1, StrategosMetasService strategosMetasService, StrategosMedicionesService strategosMedicionesService, Usuario usuario, Integer anoInicial, Integer anoFinal, Long planId)
	  {
	      ...
	      {
	    	  // 
	          respuesta = Trasladar(indicadores, fecha1, strategosMetasService, strategosMedicionesService, usuario, anoInicial, anoFinal, planId);
	          //
	      ...
	      } else
	      {
	          ...
	          TareaTrasladar t1 = new TareaTrasladar();
	          TimeUnit timeUnit;
	          long duracion;
	          if(unidadTiempo == 0)
	          {
	  ...
	  public boolean Trasladar(List<Indicador> indicadores, Calendar fecha1, StrategosMetasService strategosMetasService, StrategosMedicionesService strategosMedicionesService, Usuario usuario, Integer anoInicial, Integer anoFinal, Long planId)
	  {
	    ...
	    for (Iterator<Indicador> iter =indicadores.iterator(); iter.hasNext(); )
		{
	    	...
	    	mediciones =strategosMedicionesService.getMedicionesPeriodo(indicador.getIndicadorId(), new Long(0), anoInicial, anoInicial, 1, 12);
	    	int cont=1;
	    	
	    	for(Iterator<Medicion> iter1 =mediciones.iterator(); iter1.hasNext();){
	    		
	    		Medicion medicion = (Medicion)iter1.next();
	    		
	    		Meta metaParcial = new Meta(new MetaPK(planId, indicador.getIndicadorId(), new Long(7), (byte) 1, anoFinal, medicion.getMedicionId().getPeriodo()), medicion.getValor(), new Boolean(false));
	    		  
	    		metas.add(metaParcial);
	    		if(cont == mediciones.size()){
	    			ultimoValor = medicion.getValor();
	    		}
	    		
	    		acumulado = acumulado + medicion.getValor().intValue();
	    		cont++;
	    	}
	    	
	    	
	    	/// meta anual condiciones longitudinal
	    	if(indicador.getCorte() == 0 && indicador.getTipoCargaMedicion() == 0){
	    		Meta metaAnual = new Meta(new MetaPK(planId, indicador.getIndicadorId(), new Long(7), (byte) 2, anoFinal, 0), acumulado, new Boolean(false));
	    		metas.add(metaAnual);
	    	}
	    	else{
	    		
	    		Meta metaAnual = new Meta(new MetaPK(planId, indicador.getIndicadorId(), new Long(7), (byte) 2, anoFinal, 0), ultimoValor, new Boolean(false));
	    		metas.add(metaAnual);
	    	}
	    	
	    		
	    	strategosMetasService.saveMetas(metas, usuario);
		}	
	    respuestas = true;
