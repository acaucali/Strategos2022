...
public class ProtegerLiberarManagerMetas {
	  ...
	  //
	  public ProtegerLiberarManagerMetas(String[][] configuracion, StringBuffer log, VgcMessageResources messageResources)
	  {
	  ...
	  public ProtegerLiberarManagerMetas(PropertyCalcularManager pm, StringBuffer log, VgcMessageResources messageResources, Servicio servicio)
	  {
	    this.pm = pm;
	    this.log = log;
	    this.messageResources = messageResources;
	  ...
	  //  revisar
	  public boolean Ejecutar(String datos[][],List<Meta> metas, Calendar fecha1, StrategosMetasService strategosMetasService, byte accion, Usuario usuario)
	  {
	      ...
	      {
	    	  // 
	          respuesta = ProtegerLiberar(metas, fecha1, strategosMetasService, accion, usuario);
	          //
	  ...
	  public boolean ProtegerLiberar(List<Meta> metas, Calendar fecha1, StrategosMetasService strategosMetasService, byte accion, Usuario usuario)
	  {
	    ...
	    for (Iterator<Meta> iter =metas.iterator(); iter.hasNext(); )
		{
	    	...
	    	if (accion == 2)
			{
	    		meta.setProtegido(false);
			}
	    	if (accion == 1)
			{
	    		meta.setProtegido(true);
			}
	    	strategosMetasService.saveMeta(meta, usuario);
		}	
	    respuestas = true;
