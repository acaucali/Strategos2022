...
public class StrategosMedicionesServiceImpl
  extends StrategosServiceImpl
  implements StrategosMedicionesService
{
  private StrategosMedicionesPersistenceSession persistenceSession = null;
    
  public StrategosMedicionesServiceImpl(StrategosMedicionesPersistenceSession persistenceSession, boolean persistenceOwned, StrategosServiceFactory serviceFactory, VgcMessageResources messageResources)
  {
  ...
  public int deleteMedicion(Medicion medicion, Usuario usuario)
  {
  ...
  public int saveMedicion(Medicion medicion, Usuario usuario)
  {
    boolean transActiva = false;
    int resultado = 10000;
  ...
  public int saveMediciones(List<Medicion> mediciones, Long planId, Usuario usuario, Boolean actualizarUltimaMedicion, Boolean actualizarAlerta)
  {
    boolean transActiva = false;
    int resultado = 10000;
  ...
  private int actualizarUltimaMedicionIndicador(Indicador indicador, Long planId, Usuario usuario, StrategosIndicadoresService strategosIndicadoresService)
  {
    ...
    try
    {
      ...
      Medicion ultimaMedicion = getUltimaMedicion(indicador.getIndicadorId(), indicador.getFrecuencia(), indicador.getMesCierre(), SerieTiempo.getSerieRealId(), indicador.getValorInicialCero(), indicador.getCorte(), indicador.getTipoCargaMedicion());
  ...
  public List<Medicion> getMedicionesPeriodo(Long indicadorId, Long serieId, Integer anoDesde, Integer anoHasta, Integer periodoDesde, Integer periodoHasta)
  {
    return getMedicionesPeriodo(indicadorId, serieId, anoDesde, anoHasta, periodoDesde, periodoHasta, null);
  }
  
  public List<Medicion> getMedicionesPeriodo(Long indicadorId, Long serieId, Integer anoDesde, Integer anoHasta, Integer periodoDesde, Integer periodoHasta, Byte frecuencia)
  {
    ...
    List<Medicion> medicionesTemp = this.persistenceSession.getMedicionesPeriodo(indicadorId, serieId, anoDesde, anoHasta, periodoDesde, periodoHasta);
  ...
  public List<Medicion> getMedicionesPeriodo(Long indicadorId, Byte frecuencia, Byte mesCierre, Long serieId, Boolean valorInicialCero, Byte corte, Byte tipoMedicion, Integer anoDesde, Integer anoHasta, Integer periodoDesde, Integer periodoHasta)
  {
    if (serieId.equals(SerieTiempo.getSerieProgramadoId()))
    {
      Indicador programado = getIndicadorProgramado(indicadorId);
  ...
  public int copiarMediciones(List<Medicion> mediciones, Usuario usuario)
  {
    boolean transActiva = false;
    int resultado = 10000;
    String[] fieldNames = new String[4];
  ...
  public int actualizarSeriePorPeriodos(Integer anoInicio, Integer anoFin, Integer periodoInicio, Integer periodoFin, List<Medicion> mediciones, Boolean acumularPeriodos, Boolean desdeActividad, Usuario usuario)
  {
    boolean transActiva = false;
    int resultado = 10000;
    ...
    try
    {
      ...
      for (Iterator<Object> iter = indicadoresId.iterator(); iter.hasNext();)
      {
        ...
        Indicador indicador = (Indicador)this.persistenceSession.load(Indicador.class, indicadorId);
        List<Medicion> medicionesReales = new ArrayList();
        List<Medicion> medicionesProgramadas = new ArrayList();
        List<Medicion> medicionesProgramadasInferior = new ArrayList();
        ...
        {
          for (Iterator<Medicion> iterMedicionReal = medicionesReales.iterator(); iterMedicionReal.hasNext();)
          {
            Double ejecutado = null;
            Double programado = null;
            Double programadoInferior = null;
            Medicion medicionReal = (Medicion)iterMedicionReal.next();
            if (medicionReal.getValor() != null)
            {
              ...
              if (indicador.getCaracteristica().byteValue() != Caracteristica.getCaracteristicaCondicionBandas().byteValue())
              {
                ...
                for (Iterator<Medicion> iterMedicionProgramado = medicionesProgramadas.iterator(); iterMedicionProgramado.hasNext();)
                {
                  Medicion medicionProgramado = (Medicion)iterMedicionProgramado.next();
              ...
              else
              {
                ...
                for (Iterator<Medicion> iterMedicionProgramado = medicionesProgramadasInferior.iterator(); iterMedicionProgramado.hasNext();)
                {
                  Medicion medicionProgramado = (Medicion)iterMedicionProgramado.next();
                  if ((medicionProgramado.getMedicionId().getPeriodo().intValue() == medicionReal.getMedicionId().getPeriodo().intValue()) && 
                    (medicionProgramado.getMedicionId().getAno().intValue() == medicionReal.getMedicionId().getAno().intValue()) && (medicionProgramado.getValor() != null))
            ...
            {
              Double zonaVerde = strategosIndicadoresService.zonaVerde(indicador, medicionReal.getMedicionId().getAno(), medicionReal.getMedicionId().getPeriodo(), programado, this);
              Double zonaAmarilla = strategosIndicadoresService.zonaAmarilla(indicador, medicionReal.getMedicionId().getAno(), medicionReal.getMedicionId().getPeriodo(), programado, zonaVerde, this);
              ...
              if (index == 0)
              {
                ...
                List<Medicion> medicionesEjecutadoAnterior = getMedicionesPeriodo(indicadorId, SerieTiempo.getSerieRealId(), Integer.valueOf(anoAnterior), Integer.valueOf(anoAnterior), Integer.valueOf(periodoAnterior), Integer.valueOf(periodoAnterior), null);
                if (medicionesEjecutadoAnterior.size() > 0) {
                  ejecutadoAnterior = ((Medicion)medicionesEjecutadoAnterior.get(0)).getValor();
                }
              }
              else
              {
                ejecutadoAnterior = ((Medicion)medicionesReales.get(index - 1)).getValor();
              }
            ...
            Medicion medicion = new Medicion(new MedicionPK(medicionReal.getMedicionId().getIndicadorId(), medicionReal.getMedicionId().getAno(), medicionReal.getMedicionId().getPeriodo(), SerieTiempo.getSerieAlerta()), AlertaIndicador.ConvertToDouble(alerta), new Boolean(false));
            if (medicion.getValor() == null)
            {
              resultado = this.persistenceSession.deleteMedicion(medicion);
              if (resultado == 10001) {
        ...
        else
        {
          ...
          if (medicionesReales.size() == 0) {
            for (Iterator<Medicion> iterMedicionProgramada = medicionesProgramadas.iterator(); iterMedicionProgramada.hasNext();)
            {
              Medicion medicionProgramada = (Medicion)iterMedicionProgramada.next();
              Medicion medicion = new Medicion(new MedicionPK(medicionProgramada.getMedicionId().getIndicadorId(), medicionProgramada.getMedicionId().getAno(), medicionProgramada.getMedicionId().getPeriodo(), SerieTiempo.getSerieAlerta()), null, new Boolean(false));
              resultado = this.persistenceSession.deleteMedicion(medicion);
              if (resultado == 10001) {
                resultado = 10000;
              }
  ...
  public Medicion getUltimaMedicionConValor(List<Medicion> mediciones)
  {
    Medicion medicion = null;
    for (Iterator<Medicion> iterMedicion = mediciones.iterator(); iterMedicion.hasNext();)
    {
      Medicion med = (Medicion)iterMedicion.next();
  ...
  public Medicion getUltimaMedicionConValorEnUnPeriodo(List<Medicion> mediciones, Integer ano, Integer periodo)
  {
  ...
  public List<Medicion> getMedicionesIndicadores(List<Indicador> indicadores, Long serieId, Integer anoInical, Integer anoFinal)
  {
    ...
    List<Medicion> mediciones = this.persistenceSession.getMedicionesIndicadores(indicadoresIds, serieId, anoInical, anoFinal);
    Integer anoAnterior = anoInical != null ? Integer.valueOf(anoInical.intValue() - 1) : null;
    List<Medicion> medicionesAnoAnteriores = this.persistenceSession.getMedicionesIndicadores(indicadoresIds, serieId, anoAnterior, anoAnterior);
    
    List<Medicion> medicionesResult = new ArrayList();
    ...
    for (Iterator<Indicador> i = indicadores.iterator(); i.hasNext();)
    {
      ...
      for (Iterator<Medicion> med = mediciones.iterator(); med.hasNext();)
      {
        Medicion medicion = (Medicion)med.next();
        if (medicion.getMedicionId().getIndicadorId().longValue() == indicador.getIndicadorId().longValue())
        {
          Medicion medicionInterna = null;
          if (indicador.getCorte().byteValue() == TipoCorte.getTipoCorteLongitudinal().byteValue())
          {
            if ((indicador.getTipoCargaMedicion() != null) && (indicador.getTipoCargaMedicion().byteValue() == TipoMedicion.getTipoMedicionEnPeriodo().byteValue()))
            {
              ...
              Medicion medicionExist = this.persistenceSession.getUltimaMedicionByAnoByPeriodo(medicionesIndicador, medicion.getMedicionId().getIndicadorId(), medicion.getMedicionId().getSerieId(), anoMedicion, periodo, Boolean.valueOf(false));
  ...
  public Medicion getUltimaMedicionByAnoByPeriodo(List<Medicion> mediciones, Integer ano, Integer periodo)
  {
    Long indicadorId = ((Medicion)mediciones.get(0)).getMedicionId().getIndicadorId();
    Long serieId = ((Medicion)mediciones.get(0)).getMedicionId().getSerieId();
  ...
  public double getValorAcumulado(Long indicadorId, Long serieId) {
	...
	public List<Medicion> getMedicionList(List<Long> indicadoresIds, Long serieId, Integer anoInicial, Integer mesInicio) {
		
		return this.persistenceSession.getMedicionesList(indicadoresIds ,serieId, anoInicial, mesInicio);
	}

	
	// configuracion negativo
	
	
	public ConfiguracionNegativo getConfiguracionNegativo()
	  {
	    ConfiguracionNegativo configuracionnegativo = new ConfiguracionNegativo();
	    try
	    {
	      FrameworkService frameworkService = FrameworkServiceFactory.getInstance().openFrameworkService();
	      Configuracion configuracion = frameworkService.getConfiguracion("Strategos.Configuracion.Negativo");
	      ...
	      if (configuracion != null)
	      {
	        ...
	        configuracionnegativo.setEnviarResponsableCargarEjecutado(Boolean.valueOf(VgcAbstractService.getTagValue("cargarEjecutado", eElement).equals("1")));
	        configuracionnegativo.setEnviarResponsableCargarMeta(Boolean.valueOf(VgcAbstractService.getTagValue("cargarMeta", eElement).equals("1")));
	        configuracionnegativo.setEnviarResponsableFijarMeta(Boolean.valueOf(VgcAbstractService.getTagValue("fijarMeta", eElement).equals("1")));
	        configuracionnegativo.setEnviarResponsableLograrMeta(Boolean.valueOf(VgcAbstractService.getTagValue("lograrMeta", eElement).equals("1")));
	        configuracionnegativo.setEnviarResponsableNegativo(Boolean.valueOf(VgcAbstractService.getTagValue("validarNegativo", eElement).equals("1")));
	        configuracionnegativo.setEnviarResponsableSeguimiento(Boolean.valueOf(VgcAbstractService.getTagValue("seguimiento", eElement).equals("1")));
	        configuracionnegativo.setTexto(VgcAbstractService.getTagValue("texto", eElement));
	        configuracionnegativo.setTitulo(VgcAbstractService.getTagValue("titulo", eElement));
	      ...
	      else
	      {
	        ...
	        configuracionnegativo.setEnviarResponsableCargarEjecutado(Boolean.valueOf(true));
	        configuracionnegativo.setEnviarResponsableCargarMeta(Boolean.valueOf(true));
	        configuracionnegativo.setEnviarResponsableFijarMeta(Boolean.valueOf(true));
	        configuracionnegativo.setEnviarResponsableLograrMeta(Boolean.valueOf(true));
	        configuracionnegativo.setEnviarResponsableNegativo(Boolean.valueOf(true));
	        configuracionnegativo.setEnviarResponsableSeguimiento(Boolean.valueOf(true));
	        configuracionnegativo.setTexto("");
	        configuracionnegativo.setTitulo("");
