...
public class PlanConsolidadoReportePdfAction extends VgcReporteBasicoAction
{
	...
	private void ConsolidadoPlan(ReporteForm reporte, Document documento, HttpServletRequest request, MessageResources mensajes) throws Exception
	{
	    ...
	    Paragraph texto = new Paragraph(mensajes.getMessage("jsp.reportes.plan.consolidado.organizacion") + " : " + ((OrganizacionStrategos)request.getSession().getAttribute("organizacion")).getNombre(), font);
		...
		{
			texto = new Paragraph(mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.titulo.rango") + " : " + PeriodoUtil.getMesNombre(Byte.parseByte(reporte.getMesFinal())) + " - " + reporte.getAnoFinal(), font);
			texto.setAlignment(Element.ALIGN_CENTER);
			documento.add(texto);
			lineas = getNumeroLinea((lineas + 5), inicioLineas);
		}
		...
		if (perspectivas.size() > 0)
		{
			...
			for (Iterator<Perspectiva> iter = perspectivas.iterator(); iter.hasNext();) 
			{
				...
				SetMedicionesPerspectiva(perspectivaHija, reporte, strategosIndicadoresService, strategosMedicionesService);
				...
				if (perspectivaHija.getTipo().byteValue() == TipoPerspectiva.getTipoPerspectivaObjetivo().byteValue())
				{
					if (reporte.getVisualizarIndicadores())
						dibujarInformacionIndicador(nivel, reporte, font, perspectivaHija, documento, strategosMedicionesService, strategosIndicadoresService, strategosPerspectivasService, mensajes, request);
					if (reporte.getVisualizarIniciativas())
						dibujarInformacionIniciativa(nivel, reporte, font, perspectivaHija, documento, strategosMedicionesService, strategosIniciativasService, strategosIndicadoresService, strategosPerspectivasService, mensajes, request);
				}
				
				buildReporte(nivel, reporte, font, perspectivaHija, documento, configuracionPlan, strategosMedicionesService, strategosIniciativasService, strategosIndicadoresService, strategosPerspectivasService, mensajes, request);
			}
		}
		else if (perspectiva.getTipo().byteValue() == TipoPerspectiva.getTipoPerspectivaObjetivo().byteValue())
		{
			...
			if (reporte.getVisualizarIniciativas())
				dibujarInformacionIniciativa(nivel, reporte, font, perspectiva, documento, strategosMedicionesService, strategosIniciativasService, strategosIndicadoresService, strategosPerspectivasService, mensajes, request);
		}
	...
	private void buildReporte(int nivel, ReporteForm reporte, Font font, Perspectiva perspectiva, Document documento, ConfiguracionPlan configuracionPlan, StrategosMedicionesService strategosMedicionesService, StrategosIniciativasService strategosIniciativasService, StrategosIndicadoresService strategosIndicadoresService, StrategosPerspectivasService strategosPerspectivasService, MessageResources mensajes, HttpServletRequest request) throws Exception
	{
		...
		if (perspectivas.size() > 0)
		{
			for (Iterator<Perspectiva> iter = perspectivas.iterator(); iter.hasNext();) 
			{
				...
				buildReporte((nivel + 1), reporte, font, perspectivaHija, documento, configuracionPlan, strategosMedicionesService, strategosIniciativasService, strategosIndicadoresService, strategosPerspectivasService, mensajes, request);
			}
		}
	}
	...
	private void dibujarInformacionIndicador(int nivel, ReporteForm reporte, Font font, Perspectiva perspectiva, Document documento, StrategosMedicionesService strategosMedicionesService, StrategosIndicadoresService strategosIndicadoresService, StrategosPerspectivasService strategosPerspectivasService, MessageResources mensajes, HttpServletRequest request) throws Exception
	{
		...
		if (indicadores.size() > 0)
		{
			...
			java.awt.Font fontNormal = new java.awt.Font(font.getFamilyname(), font.style(), ((Float) (font.size())).intValue());
			...
			for (Iterator<Indicador> iter = indicadores.iterator(); iter.hasNext();)
			{
				...
				if (indicador.getFechaUltimaMedicion() != null) 
				{
					...
					if (metas.size() > 0)
					{
						...
						celda = (indicador.getMetaAnual() != null ? VgcFormatter.formatearNumero(indicador.getMetaAnual()) : "");
					}
				}
				contador++;
				fila[contador][0] = celda;
				...
				if (indicador.getResponsableLograrMetaId() != null)
					celda = (indicador.getResponsableLograrMeta() != null ? indicador.getResponsableLograrMeta().getNombreCargo() : "");
				contador++;
				fila[contador][0] = celda;
				fila[contador][1] = TablaBasicaPDF.H_ALINEACION_CENTER;
				fila[contador][2] = metrics.stringWidth((String)fila[contador][0]);
		...
		else
		{
			...
			texto = new Paragraph(mensajes.getMessage("jsp.reportes.plan.consolidado.reporte.noindicadores", reporte.getPlantillaPlanes().getNombreIndicadorPlural().toLowerCase(), new com.visiongc.app.strategos.web.struts.reportes.actions.PlanEjecucionReporteAction().getNombrePerspectiva(reporte, (nivel)).toLowerCase()), font);
	...
	private void dibujarInformacionIniciativa(int nivel, ReporteForm reporte, Font font, Perspectiva perspectiva, Document documento, StrategosMedicionesService strategosMedicionesService, StrategosIniciativasService strategosIniciativasService, StrategosIndicadoresService strategosIndicadoresService, StrategosPerspectivasService strategosPerspectivasService, MessageResources mensajes, HttpServletRequest request) throws Exception
	{
		StrategosPlanesService strategosPlanesService = StrategosServiceFactory.getInstance().openStrategosPlanesService();
		StrategosExplicacionesService strategosExplicacionesService = StrategosServiceFactory.getInstance().openStrategosExplicacionesService();
		Map<String, Object> filtros = new HashMap<String, Object>();
		...
		if (iniciativas.size() > 0)
		{
    		...
    		contador++;
		    columnas[contador][0] = "20";
		    columnas[contador][1] = mensajes.getMessage("jsp.reportes.plan.consolidado.reporte.columna.iniciativa.responsable");
			...
			for (Iterator<Iniciativa> iter = iniciativas.iterator(); iter.hasNext();)
			{
  			  	...
  			  	if (indicadorId != null)
  			  	{
	  			  	...
	  			  	if (reporte.getTipoPeriodo().byteValue() == reporte.getPeriodoAlCorte())
	  			  	{
	  	  			  	if (indicadorPresupuesto.getTipoCargaMedicion().byteValue() == TipoMedicion.getTipoMedicionEnPeriodo().byteValue())
	  	  			  	{
	  				  		...
	  				  		//List<Medicion> medicionesProgramada = strategosMedicionesService.getMedicionesPeriodo(indicadorPresupuesto.getIndicadorId(), SerieTiempo.getSerieProgramado().getSerieId(), new Integer(0000), new Integer(9999), new Integer(000), new Integer(999));
	  			  	...
	  			  	else
	  			  	{
	  	  			  	if (indicadorPresupuesto.getTipoCargaMedicion().byteValue() == TipoMedicion.getTipoMedicionEnPeriodo().byteValue())
	  	  			  	{
	  				  		...
	  				  		//List<Medicion> medicionesProgramada = strategosMedicionesService.getMedicionesPeriodo(indicadorPresupuesto.getIndicadorId(), SerieTiempo.getSerieProgramado().getSerieId(), new Integer(0000), Integer.parseInt(reporte.getAnoInicial()), new Integer(000), Integer.parseInt(reporte.getMesInicial()));
				...
				if (iniciativa.getResponsableLograrMetaId() != null)
					celda = (iniciativa.getResponsableLograrMeta() != null ? iniciativa.getResponsableLograrMeta().getNombreCargo() : "");
				contador++;
				fila[contador][0] = celda;
				fila[contador][1] = TablaBasicaPDF.H_ALINEACION_CENTER;
				fila[contador][2] = metrics.stringWidth((String)fila[contador][0]);
		...
		{
			font.setColor(0, 0, 255);
			texto = new Paragraph(mensajes.getMessage("jsp.reportes.plan.consolidado.reporte.noindicadores", reporte.getPlantillaPlanes().getNombreIniciativaPlural().toLowerCase(), new com.visiongc.app.strategos.web.struts.reportes.actions.PlanEjecucionReporteAction().getNombrePerspectiva(reporte, (nivel)).toLowerCase()), font);
			texto.setIndentationLeft(50);
			documento.add(texto);
			font.setColor(0, 0, 0);
			lineas = getNumeroLinea(lineas, inicioLineas);
	...
	public void SetMedicionesPerspectiva(Perspectiva perspectiva, ReporteForm reporte, StrategosIndicadoresService strategosIndicadoresService, StrategosMedicionesService strategosMedicionesService)
	{
		if (reporte.getTipoPeriodo().byteValue() == reporte.getPeriodoPorPeriodo().byteValue())
		{
			// Buscar Medicion de Indicador Parcial
			Indicador indicador = (Indicador)strategosIndicadoresService.load(Indicador.class, perspectiva.getNlParIndicadorId());
			if (indicador != null)
			{
		    	LapsoTiempo lapsoTiempoEnPeriodos = PeriodoUtil.getLapsoTiempoEnPeriodosPorMes(((Integer)(Integer.parseInt(reporte.getAnoInicial()))).intValue(), ((Integer)(Integer.parseInt(reporte.getAnoFinal()))).intValue(), ((Integer)(Integer.parseInt(reporte.getMesInicial()))).intValue(), ((Integer)(Integer.parseInt(reporte.getMesFinal()))).intValue(), indicador.getFrecuencia().byteValue());
		    	int periodoInicio = lapsoTiempoEnPeriodos.getPeriodoInicio().intValue();
			...
			indicador = (Indicador)strategosIndicadoresService.load(Indicador.class, perspectiva.getNlAnoIndicadorId());
			if (indicador != null)
			{
		    	LapsoTiempo lapsoTiempoEnPeriodos = PeriodoUtil.getLapsoTiempoEnPeriodosPorMes(((Integer)(Integer.parseInt(reporte.getAnoInicial()))).intValue(), ((Integer)(Integer.parseInt(reporte.getAnoFinal()))).intValue(), ((Integer)(Integer.parseInt(reporte.getMesInicial()))).intValue(), ((Integer)(Integer.parseInt(reporte.getMesFinal()))).intValue(), indicador.getFrecuencia().byteValue());
		    	int periodoInicio = lapsoTiempoEnPeriodos.getPeriodoInicio().intValue();
