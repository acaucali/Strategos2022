...
public class ImportarActividadesAction extends VgcAction{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if (presentacion != null && presentacion.getData() != null)
			importarActividadesForm.setShowPresentacion(presentacion.getData().equals("1") ? true : false);
		...
		else
		{
			...
			if (!new FrameworkConnection().testConnection(url, driver, user, new com.visiongc.framework.web.struts.forms.servicio.GestionarServiciosForm().getPasswordConexionDecriptado(password)))
			{
				...
				importarActividadesForm.setStatus(ImportarStatus.getImportarStatusSuccess());
		}
		
		importarActividadesForm.setTipoFuente(ImportacionType.getImportacionTypePlano());				
		importarActividadesForm.setSeparador("|");	
		importarActividadesForm.setLogMediciones(true);
		importarActividadesForm.setLogErrores(true);
		importarActividadesForm.setFileForm(null);		
