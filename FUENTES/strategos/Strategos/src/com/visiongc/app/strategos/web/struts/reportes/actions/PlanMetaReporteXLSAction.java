		...
		if (indicadores.size() > 0)
		{
			...
			for (Iterator<Indicador> iter = indicadores.iterator(); iter.hasNext();)
			{
				...
				if (reporte.getAcumular())
				{
					...
					mediciones = strategosMedicionesService.getMedicionesPorFrecuencia(indicador.getIndicadorId(), SerieTiempo.getSerieRealId(), new Integer(reporte.getAnoInicial()), reporte.getAno(), periodoInicio, periodoFin, indicador.getFrecuencia(), indicador.getFrecuencia(), acumular, acumular);
				}
				...
				else
				{
					...
					if (medicionesMetas.size() > 0)
					{
						if (!esAcumulado)
						{
							...
							{
								Medicion medicion = iterMed.next();
