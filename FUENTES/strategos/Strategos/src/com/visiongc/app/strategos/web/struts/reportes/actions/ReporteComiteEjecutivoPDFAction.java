...
public class ReporteComiteEjecutivoPDFAction extends VgcReporteBasicoAction
{
	 ...
	 *
	 * Inserta un t�tulo en el reporte.
	 *
	    ...
	    if (vista == 2)
		{
	    	...
	    	for (String indicadore : indicadores) {
	        	...
	        	if (lstClasesActivos.size() > 0)
	        	{
			    	...
			    	tabla = GenerarTablaVistaBanca(documento, lstClasesActivos, organizaciones, indicador, vista, "", "Activos (Cartera de Cr�ditos)", "Cartera de Cr�ditos", diaReporte, mesReporte, anoReporte, request);
			    	documento.add(tabla.getTabla());
			    	haypages = true;
	        	}
		    ...
		    for (String organizacione : organizaciones) {
		    	for (String indicadore : indicadores) {
		        	...
		        	if (lstClasesPasivos.size() > 0)
		        	{
					    ...
					    //Tabla Pasivos
					    tabla = GenerarTabla(documento, lstClasesPasivos, new Long(organizacione), indicador, vista, "Capt. P�blico + Partipaciones", "Pasivos (Captaciones)", "Captaciones", diaReporte, mesReporte, anoReporte, mesCierre, request, true);
				    	documento.add(tabla.getTabla());
				    	haypages = true;
		        	}
		        	...
		        	if (lstClasesActivos.size() > 0)
		        	{
				    	...
				    	tabla = GenerarTabla(documento, lstClasesActivos, new Long(organizacione), indicador, vista, "Cartera Cr�ditos Bruta", "Activos (Cartera de Cr�ditos)", "Cartera de Cr�ditos", diaReporte, mesReporte, anoReporte, mesCierre, request, false);
				    	documento.add(tabla.getTabla());
				    	haypages = true;
		        	}
	    ...
	    if (vista == 1)
	    {
	    	...
	    	for (ClaseIndicadores claseA : lista) {
	    		if (claseA.getOrganizacionId().equals(organizacionId) && !claseA.getNombre().equals(tituloComparacion))
	    		{
	    			...
	    			tabla.setAlineacionHorizontal(Tabla.H_ALINEACION_LEFT);
	    			if (claseA.getNombre().trim().equalsIgnoreCase("Activos (Cartera de Cr�ditos)") || claseA.getNombre().trim().equalsIgnoreCase("Pasivos (Captaciones)"))
	    			{
	    			...
	    			if (claseA.getNivel() != null && ((_minimoNivelActivo != null && claseA.getNivel().intValue() == _minimoNivelActivo && tituloCol == "Activos (Cartera de Cr�ditos)") || (_minimoNivelPasivo != null && claseA.getNivel().intValue() == _minimoNivelPasivo && tituloCol == "Pasivos (Captaciones)")))
	...
	private Boolean ClaseEsActivo(ClaseIndicadores clase)
	{
		...
		if (clase.getNombre().trim().equalsIgnoreCase("Activos (Cartera de Cr�ditos)"))
			resultado = true;
	...
	private TablaBasicaPDF PrintEncabezado(String tituloCol, HttpServletRequest request, Font font, boolean saltarPagina, Document documento, TablaBasicaPDF tabla, int vista, int mesCierre, int anoReporte, int mesReporte, String tituloFechaAntiayer, String tituloFechaAntier, String tituloFechaAyer, String tituloFechaBase) throws Exception
	{
	    ...
	    String tituloColumnas[] = {titulo2, NombreMes(mesCierre, anoReporte - 1, true), NombreMes(mesReporte - 1, anoReporte, true), tituloFechaAntiayer, tituloFechaAntier, tituloFechaAyer, tituloFechaBase, "D�a", "Mes", "A�o", "Var Mes", "Crec Mes", "Desv Abs", "Cump. %"};
