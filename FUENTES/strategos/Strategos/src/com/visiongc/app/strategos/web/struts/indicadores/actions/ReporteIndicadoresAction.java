...
public class ReporteIndicadoresAction extends VgcReporteBasicoAction
{
	...
	protected void construirReporte(ActionForm form, HttpServletRequest request, HttpServletResponse response, Document documento) throws Exception
	{
		...
		if (indicadores.size() > 0)
		{
			...
			font.setSize(10.0F);
			Paragraph texto = new Paragraph("N�mero de Indicadores: " + Integer.toString(indicadores.size()), font);
			texto.setAlignment(0);
			documento.add(texto);
		}
		else
		{
			font.setSize(10.0F);
			Paragraph texto = new Paragraph("No hay indicadores con las caracter�sticas solicitadas", font);
			texto.setAlignment(0);
