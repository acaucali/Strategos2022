...
public class ReporteInstrumentosPdf extends VgcReporteBasicoAction
{
	...
	private void dibujarInformacionActividad(ReporteForm reporte, Font font, Iniciativa iniciativa, Document documento, StrategosMedicionesService strategosMedicionesService, StrategosIndicadoresService strategosIndicadoresService, MessageResources mensajes, HttpServletRequest request) throws Exception
	{
		...
		if (actividades.size() > 0)
		{
			...
			tabla.agregarCelda("\n" + mensajes.getMessage("jsp.asignarpesosactividad.actividad"));
			tabla.agregarCelda("\n" + mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.titulo.inicio"));
			...
			tabla.agregarCelda("\n" + mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.titulo.alerta"));
	...
	private void crearTablaActividad(ReporteForm reporte, PryActividad actividad, Indicador indicador, Font font, MessageResources mensajes, Document documento, HttpServletRequest request, Tabla tabla) throws Exception
	{
		
		
		
		if(actividad.getNombre() != null && !actividad.getNombre().equals("")){
			tabla.agregarCelda("\n" + actividad.getNombre());
		}else
			tabla.agregarCelda("");
			
		if(actividad.getComienzoPlan() != null)
			tabla.agregarCelda(VgcFormatter.formatearFecha(actividad.getComienzoPlan(),"formato.fecha.corta") );
		else
			tabla.agregarCelda("");

		if(actividad.getFinPlan() != null)
			tabla.agregarCelda(VgcFormatter.formatearFecha(actividad.getFinPlan(),"formato.fecha.corta") );
		else
			tabla.agregarCelda("");
	    
		if(actividad.getDuracionPlan() != null)
