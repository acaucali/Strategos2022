...
public class GestionarIniciativasAction extends VgcAction {
	@Override
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		boolean actualizarForma = request.getParameter("actualizarForma") != null
				? Boolean.parseBoolean(request.getParameter("actualizarForma"))
				: false;
		if (actualizarForma) {
		...
		GestionarPortafoliosForm gestionarPortafoliosForm = (GestionarPortafoliosForm) request.getSession()
				.getAttribute("gestionarPortafoliosForm");
		...
		GestionarInstrumentosForm gestionarInstrumentoForm = (GestionarInstrumentosForm) request.getSession()
				.getAttribute("gestionarInstrumentosForm");
		if ((gestionarInstrumentoForm != null)
				&& (forward.equals("gestionarInstrumentosIndicadoresIniciativasAction"))) {
			instrumentos = true;
		...
		Byte filtroHistorycoType = (gestionarIniciativasForm != null) && (gestionarIniciativasForm.getFiltro() != null)
				? gestionarIniciativasForm.getFiltro().getHistorico()
				: null;

		String filtroNombre = "";

		if (gestionarIniciativasForm != null && gestionarIniciativasForm.getFiltro() != null
				&& gestionarIniciativasForm.getFiltroNombre() != null) {
		...
		} else if ((gestionarPortafoliosForm != null) && (gestionarPortafoliosForm.getIniciativaId() != null)
				&& (gestionarIniciativasForm.getSource().equals("Portafolio"))) {
			iniciativaId = gestionarPortafoliosForm.getIniciativaId();
		} else if ((gestionarInstrumentoForm != null) && (gestionarInstrumentoForm.getIniciativaId() != null)
				&& (gestionarIniciativasForm.getSource().equals("instrumentos"))) {
			iniciativaId = gestionarInstrumentoForm.getIniciativaId();
		}

		Long planId = (request.getParameter("planId") != null) && (request.getParameter("planId") != "")
				? Long.valueOf(Long.parseLong(request.getParameter("planId")))
				: null;
		...
		String nombre = filtroNombre != null ? filtroNombre
				: request.getParameter("filtroNombre") != null ? request.getParameter("filtroNombre") : "";
		String anio = filtroAnio != null ? filtroAnio
				: request.getParameter("filtroAnio") != null ? request.getParameter("filtroAnio") : "";
		...
		Long selectEstatusType = (request.getParameter("selectEstatusType") != null)
				&& (request.getParameter("selectEstatusType") != "")
				&& (!request.getParameter("selectEstatusType").equals("0"))
						? Long.valueOf(Long.parseLong(request.getParameter("selectEstatusType")))
						: null;
		Long selectTipos = (request.getParameter("selectTipos") != null) && (request.getParameter("selectTipos") != "")
				&& (!request.getParameter("selectTipos").equals("0"))
						? Long.valueOf(Long.parseLong(request.getParameter("selectTipos")))
						: null;

		if (!nombre.equals(""))
			request.getSession().setAttribute("nombreIniciativa", nombre);
		if (!anio.equals(""))
			request.getSession().setAttribute("anioIniciativa", anio);
		if (selectEstatusType != null)
			request.getSession().setAttribute("selectEstatusTypeIniciativa", selectEstatusType);
		if (selectTipos != null)
			request.getSession().setAttribute("selectTiposIniciativa", selectTipos);

		if (request.getParameter("limpiarFiltros") != null) {
			request.getSession().setAttribute("nombreIniciativa", null);
			request.getSession().setAttribute("anioIniciativa", null);
			request.getSession().setAttribute("selectEstatusTypeIniciativa", null);
			request.getSession().setAttribute("selectTiposIniciativa", null);
		}

		String nombreAttribute = "";
		String anioAttribute = "";
		Long selectEstatusTypeAttribute = null;
		Long selectTiposAttribute = null;

		if (request.getSession().getAttribute("nombreIniciativa") != null)
			nombreAttribute = (String) request.getSession().getAttribute("nombreIniciativa");
		else
			nombreAttribute = "";

		if (request.getSession().getAttribute("anioIniciativa") != null)
			anioAttribute = (String) request.getSession().getAttribute("anioIniciativa");
		else
			anioAttribute = "";

		if (request.getSession().getAttribute("selectEstatusTypeIniciativa") != null)
			selectEstatusTypeAttribute = (Long) request.getSession().getAttribute("selectEstatusTypeIniciativa");
		else
			selectEstatusTypeAttribute = null;

		if (request.getSession().getAttribute("selectTiposIniciativa") != null)
			selectTiposAttribute = (Long) request.getSession().getAttribute("selectTiposIniciativa");
		else
			selectTiposAttribute = null;

		FiltroForm filtro = new FiltroForm();
		filtro.setHistorico(selectHitoricoType);
		if (nombreAttribute.equals("")) {
			filtro.setNombre(null);
		} else {
			filtro.setNombre(nombreAttribute);
		}
		if (anioAttribute.equals("")) {
			filtro.setAnio(null);
		} else {
			filtro.setAnio(anioAttribute);
		}

		gestionarIniciativasForm.setFiltro(filtro);
		gestionarIniciativasForm.setEstatus(selectEstatusTypeAttribute);
		gestionarIniciativasForm.setTipo(selectTiposAttribute);
		...
		gestionarIniciativasForm
				.setEditarForma(Boolean.valueOf(getPermisologiaUsuario(request).tienePermiso("INICIATIVA_EDIT")));
		if (source != null) {
		...
		if (iniciativaId != null) {
			...
			Iniciativa iniciativaSeleccionada = (Iniciativa) strategosIniciativasService.load(Iniciativa.class,
					new Long(gestionarIniciativasForm.getSeleccionadoId()));
		...
		if ((organizacionId != null) && (!organizacionId.equals("")) && (gestionarIniciativasForm.getSource() != null)
				&& (!gestionarIniciativasForm.getSource().equals("Plan"))) {
			filtros.put("organizacionId", organizacionId);
		}
		if ((planId != null) && (planId.longValue() != 0L)) {
			filtros.put("planId", planId.toString());
		}
		if ((gestionarIniciativasForm.getIniciativaId() != null) && (gestionarIniciativasForm.getSource() != null)
				&& (gestionarIniciativasForm.getSource().equals("Plan"))
				&& (gestionarIniciativasForm.getPlanId() == null)) {
			filtros.put("iniciativaId", gestionarIniciativasForm.getIniciativaId().toString());
		}

		if (gestionarIniciativasForm.getFiltro().getHistorico().byteValue() == HistoricoType
				.getFiltroHistoricoNoMarcado())
		...
		PaginaLista paginaIniciativas = strategosIniciativasService.getIniciativas(paginaIniciativa, 30,
				gestionarIniciativasForm.getAtributoOrden(), gestionarIniciativasForm.getTipoOrden(), true, filtros);
		...
		if (paginaIniciativas.getLista().size() > 0) {
			...
			if ((gestionarIniciativasForm.getSeleccionadoId() == null)
					|| (gestionarIniciativasForm.getSeleccionadoId().equals(""))) {
			...
			if (iniciativaSeleccionada != null) {
				...
				for (Iterator<IniciativaPlan> iter = iniciativaPlanes.iterator(); iter.hasNext();) {
					...
					if (!listaPlanes.contains(iniciativaPlan.getPlan())) {
						...
						OrganizacionStrategos organizacion = (OrganizacionStrategos) strategosPlanesService
								.load(OrganizacionStrategos.class, plan.getOrganizacionId());
				...
				iniciativaId = ((Iniciativa) paginaIniciativas.getLista().get(0)).getIniciativaId();
				gestionarIniciativasForm.setSeleccionadoId(iniciativaId.toString());
				iniciativaSeleccionada = (Iniciativa) strategosIniciativasService.load(Iniciativa.class,
						new Long(gestionarIniciativasForm.getSeleccionadoId()));
				if (iniciativaSeleccionada != null) {
					gestionarIniciativasForm.setNombreIniciativaSeleccionada(iniciativaSeleccionada.getNombre());
					gestionarIniciativasForm.setTipoAlerta(iniciativaSeleccionada.getTipoAlerta());
					...
					for (Iterator<IniciativaPlan> iter = iniciativaPlanes.iterator(); iter.hasNext();) {
						...
						if (!listaPlanes.contains(iniciativaPlan.getPlan())) {
							Plan plan = (Plan) strategosPlanesService.load(Plan.class,
									iniciativaPlan.getPk().getPlanId());
							OrganizacionStrategos organizacion = (OrganizacionStrategos) strategosPlanesService
									.load(OrganizacionStrategos.class, plan.getOrganizacionId());
							plan.setOrganizacion(organizacion);
				...
				if (!forward.equals("gestionarPortafoliosIndicadoresIniciativaAction")) {
					...
					if ((gestionarIniciativasForm.getSource() != null)
							&& (gestionarIniciativasForm.getSource().equals("Plan"))) {
				...
				messages.add("org.apache.struts.action.GLOBAL_MESSAGE",
						new ActionMessage("action.gestionariniciativas.iniciativaeliminada"));
			...
			gestionarIniciativasForm
					.setTipoAlerta(TipoCalculoEstadoIniciativa.getTipoCalculoEstadoIniciativaPorActividades());
		}
	...
	public String obtenerObjetivo(Long iniciativaId, Long planId) throws SQLException {
		...
		if (id != null && planId != null) {
			...
			if ((ini.getIniciativaPerspectivas() != null) && (ini.getIniciativaPerspectivas().size() > 0)) {
				...
				IniciativaPerspectiva iniciativaPerspectiva = (IniciativaPerspectiva) ini.getIniciativaPerspectivas()
						.toArray()[0];
