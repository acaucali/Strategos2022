...
--DROP FUNCTION getCalcularAlertaIniciativa();

CREATE OR REPLACE FUNCTION getCalcularAlertaIniciativa() RETURNS SETOF iniciativa AS
$BODY$
DECLARE
...
BEGIN
    FOR r IN SELECT iniciativa_id FROM iniciativa
    WHERE iniciativa_id > 0
    LOOP
	SELECT INTO f * FROM iniciativa WHERE iniciativa_id = r.iniciativa_id LIMIT 1;
	...
	zv := f.alerta_zv;
	IF za IS NULL THEN
	   SELECT INTO za alerta_iniciativa_za FROM organizacion WHERE organizacion_id = f.organizacion_id LIMIT 1;
	END IF;
	IF zv IS NULL THEN
	   SELECT INTO zv alerta_iniciativa_zv FROM organizacion WHERE organizacion_id = f.organizacion_id LIMIT 1;
	END IF;
	...
	WHERE iniciativa_id = r.iniciativa_id;
    END LOOP;
    RETURN;
END
$BODY$
...
SELECT getCalcularAlertaIniciativa();
